...
const Dashboard = () => {
  ...
  const weekTotalMinutes = weekBreaks.reduce((sum, b) => sum + getMinutes(b.start, b.end), 0);  // Statistiques des activités
  const todayActivities = recentActivitySessions.filter((s: ActivitySession) => isToday(s.date));
  const weekActivities = recentActivitySessions.filter((s: ActivitySession) => isThisWeek(s.date));
  
  const avgStressReduction = recentActivitySessions.length > 0
    ? recentActivitySessions.reduce((sum: number, s: ActivitySession) => sum + (s.feedback.stressBefore - s.feedback.stressAfter), 0) / recentActivitySessions.length
    : 0;

  const avgEnergyGain = recentActivitySessions.length > 0
    ? recentActivitySessions.reduce((sum: number, s: ActivitySession) => sum + (s.feedback.energyAfter - s.feedback.energyBefore), 0) / recentActivitySessions.length
    : 0;

  const avgMoodImprovement = recentActivitySessions.length > 0
    ? recentActivitySessions.reduce((sum: number, s: ActivitySession) => sum + (s.feedback.moodAfter - s.feedback.moodBefore), 0) / recentActivitySessions.length
    : 0;

  const avgConcentration = recentActivitySessions.length > 0
    ? recentActivitySessions.reduce((sum: number, s: ActivitySession) => sum + s.feedback.concentration, 0) / recentActivitySessions.length
    : 0;

  const avgMotivation = recentActivitySessions.length > 0
    ? recentActivitySessions.reduce((sum: number, s: ActivitySession) => sum + s.feedback.motivation, 0) / recentActivitySessions.length
    : 0;
      ...
      <PageContainer>
        ...
        <div className="w-full bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-[#7346FF] to-[#9C5AFF] text-white p-8">
            <h1 className="text-3xl font-bold mb-2">Tableau de bord personnalisé</h1>
            <p className="text-purple-100 text-lg">Suivi complet de vos pauses et activités bien-être</p>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="mb-12">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800">📊 Vue d'ensemble aujourd'hui</h2>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-blue-200 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-semibold text-blue-700">Pauses</h3>
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl border border-green-200 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-semibold text-green-700">Activités</h3>
                    <span className="text-2xl">🎯</span>
                  </div>
                  <p className="text-3xl font-bold text-green-800">{todayActivities.length}</p>
                  <p className="text-sm text-green-600">{todayActivities.reduce((sum: number, a: ActivitySession) => sum + a.duration, 0)} min total</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-xl border border-purple-200 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-semibold text-purple-700">Durée moy.</h3>
                    <span className="text-2xl">📏</span>
                  </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-6 rounded-xl border border-orange-200 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-semibold text-orange-700">Concentration</h3>
                    <span className="text-2xl">🧠</span>
                  </div>
                  <p className="text-3xl font-bold text-orange-800">{avgConcentration.toFixed(1)}</p>
            ...
            <div className="mb-12">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800">💪 Impact sur votre bien-être</h2>
              </div>
              ...
              <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center p-6 bg-white rounded-lg shadow-sm border border-red-100">
                    ...
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Réduction stress</h3>
                    <p className="text-3xl font-bold text-red-600">
                      {avgStressReduction > 0 ? '-' : ''}
                      {Math.abs(avgStressReduction).toFixed(1)}
                    </p>
                    <p className="text-sm text-gray-500 mt-1">points en moyenne</p>
                  </div>
                  
                  <div className="text-center p-6 bg-white rounded-lg shadow-sm border border-green-100">
                    <div className="text-4xl mb-3">⚡</div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Gain d'énergie</h3>
                  ...
                  <div className="text-center p-6 bg-white rounded-lg shadow-sm border border-blue-100">
                    <div className="text-4xl mb-3">😊</div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Amélioration humeur</h3>
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-100">
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-blue-800">Motivation moyenne</p>
                      <p className="text-lg font-bold text-blue-900">{avgMotivation.toFixed(1)}/10</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mb-12">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800">📈 Résumé de la semaine</h2>
              </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl p-6 border border-blue-200">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-900">{weekTotalPauses}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-blue-700">Temps total</span>
                      <span className="font-bold text-blue-900">{weekTotalMinutes} min</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-blue-700">Moyenne/jour</span>
                      <span className="font-bold text-blue-900">{Math.round(weekTotalPauses / 7)} pauses</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl p-6 border border-green-200">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-green-900">{weekActivities.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-green-700">Temps total</span>
                      <span className="font-bold text-green-900">{weekActivities.reduce((sum: number, a: ActivitySession) => sum + a.duration, 0)} min</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-green-700">Activités populaires</span>
                      <span className="font-bold text-green-900">{activities.length} types</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              {recentActivitySessions.length > 0 ? (
                <div className="space-y-4">
                  {recentActivitySessions.slice(0, 3).map((session: ActivitySession) => {
                    const activity = activities.find(a => a.id === session.activityId);
                    return (
                      <div key={session.id} className="bg-gradient-to-r from-gray-50 to-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-200">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex-1">
                            <h3 className="font-bold text-lg text-gray-800">{activity?.title || 'Activité'}</h3>
                            <p className="text-gray-600 mb-2">{activity?.description || 'Description non disponible'}</p>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-100">
                          <div className="text-center">
                            ...
                            <p className="font-bold text-red-600">{session.feedback.stressBefore} → {session.feedback.stressAfter}</p>
                          </div>
                          <div className="text-center">
                          ...
                          <div className="text-center">
                            ...
                            <p className="font-bold text-blue-600">{session.feedback.moodBefore} → {session.feedback.moodAfter}</p>
                          </div>
                          <div className="text-center">
                ...
                <div className="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
                  ...
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">Aucune activité récente</h3>
                  <p className="text-gray-500 text-lg mb-6">Commencez votre première activité pour voir vos statistiques ici !</p>
                  <div className="bg-white p-4 rounded-lg inline-block border border-gray-200">
                    <p className="text-sm text-gray-600">💡 Astuce : Visitez la page Activités pour découvrir tous les exercices disponibles</p>
                  </div>
                </div>
              )}
            </div>
                ...
                {recentActivitySessions.slice(0, 3).map((session: ActivitySession) => {
                  ...
                  <div key={session.id} className="bg-gradient-to-r from-gray-50 to-white p-6 rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="font-semibold text-gray-800">{activity?.title || 'Activité'}</h3>
